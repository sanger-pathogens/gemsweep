...
include { SKANI_DEREP } from './subworkflows/skani_derep.nf'
...
workflow {
    ...
    if (params.references) {
        ...
        if (params.refine_refs) {
            if params.refine_method == "network_distance" {
            ...
            } else if (params.refine_method == "skani") {
                ref_db_files_ch = channel.fromPath(params.ref_db_files).first()

                SKANI_DEREP(references_ch, ref_db_files_ch)
                SKANI_DEREP.out.filtered_ref_ch
                | set { filtered_ref_ch }

                PREP_REFS(filtered_ref_ch)
                POPPUNK(PREP_REFS.out.refs_csv)
                poppunk_clusters_csv = POPPUNK.out.clusters

                ref_groups_ch = ORDER_GROUPS(PREP_REFS.out.refs_csv, poppunk_clusters_csv).groups
                representatives_ch = filtered_ref_ch
            }
        } else {
